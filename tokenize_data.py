...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="test",
        help="The split of the dataset to use. Default is 'test'.",
    )
    parser.add_argument(
        "--subset",
        type=str,
    ...
    parser.add_argument(
        ...
        default="input",
        type=str,
        help="The column name for the text data. Default is 'input'.",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
...
def tokenize(example, tokenizer, col="input"):
    ...
    example["tokens"] = tokenizer.batch_decode(
...
def main(args):
    ...
    dataset = utils.load_data_hf(
        args.dataset, args.split, args.subset, args.start_index, args.num_samples
    )
    if args.dataset == "emozilla/govreport-test-tokenized":
        dataset = dataset.remove_columns(
            ["input_ids", "attention_mask", "output", "tokenized_len"]
        )
    ...
    dataset = (
        dataset.remove_columns([args.text_col]) if args.text_col != "input" else dataset
    )
    path = os.path.join(
        args.save_dir, f"{args.start_index}_{args.num_samples}_{args.split}"
    )
    if args.subset:
